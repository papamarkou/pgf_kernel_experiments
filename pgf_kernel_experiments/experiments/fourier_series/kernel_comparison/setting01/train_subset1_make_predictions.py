...
for i in range(runner.num_gps()):
    runner.single_runners[i].model.load_state_dict(
        torch.load(output_path.joinpath('train_subset1_'+kernel_names[i]+'_gp_state.pth'))
    )
...
np.savetxt(
    output_path.joinpath('train_subset1_predictions.csv'),
...
np.savetxt(
    output_path.joinpath('train_subset1_error_metrics.csv'),
    scores.detach().numpy(),
    delimiter=',',
