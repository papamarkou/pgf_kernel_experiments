...
for i in range(runner.num_gps()):
    runner.single_runners[i].model.load_state_dict(
        torch.load(output_path.joinpath('train_subset3_'+kernel_names[i]+'_gp_state.pth'))
    )
...
np.savetxt(
    output_path.joinpath('train_subset3_predictions.csv'),
...
np.savetxt(
    output_path.joinpath('train_subset3_error_metrics.csv'),
    scores.detach().numpy(),
    delimiter=',',
