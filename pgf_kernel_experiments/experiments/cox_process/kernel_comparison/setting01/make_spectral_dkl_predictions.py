...
for run_count in range(num_runs):
    ...
    kernel = gpytorch.kernels.ScaleKernel(
        gpytorch.kernels.SpectralMixtureKernel(num_mixtures=6, ard_num_dims=3, batch_shape=torch.Size((num_classes,))),
    ...
    runner.model.load_state_dict(torch.load(output_paths[run_count].joinpath('spectral_dkl_state.pth')))
    ...
    np.savetxt(
        output_paths[run_count].joinpath('spectral_dkl_predictions.csv'),
    ...
    np.savetxt(
        output_paths[run_count].joinpath('spectral_dkl_error_metrics.csv'),
...
np.savetxt(
    output_basepath.joinpath('spectral_dkl_error_metric_means.csv'),
...
np.savetxt(
    output_basepath.joinpath('spectral_dkl_error_metric_stds.csv'),
