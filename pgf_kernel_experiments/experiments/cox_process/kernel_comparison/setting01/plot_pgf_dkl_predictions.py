...
# %% Generate and save plots of predictions with PGF kernel
...
for i in range(num_runs):
    ...
    projected_x = np.loadtxt(output_paths[i].joinpath('pgf_dkl_projections.csv'), delimiter=',')
    ...
    plt.savefig(
        output_paths[i].joinpath('pgf_dkl_predictions.pdf'),
        dpi=dpi,
        bbox_inches='tight',
        pad_inches=0.1
    )
