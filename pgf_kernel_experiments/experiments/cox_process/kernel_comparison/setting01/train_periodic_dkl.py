...
while ((success_count < num_runs) and (tot_count < num_train_seeds)):
    ...
    try:
        ...
        torch.save(runner.model.state_dict(), output_paths[success_count].joinpath('periodic_dkl_state.pth'))

        # Save losses

        np.savetxt(
            output_paths[success_count].joinpath('periodic_dkl_losses.csv'),
        ...
        np.savetxt(
            output_paths[success_count].joinpath('periodic_dkl_projections.csv'),
...
np.savetxt(
    output_basepath.joinpath('periodic_dkl_successful_seeds.csv'),
    np.array(successful_seeds),
    fmt='%i'
)

np.savetxt(
    output_basepath.joinpath('periodic_dkl_failed_seeds.csv'),
