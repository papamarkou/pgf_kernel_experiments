...
# %% Generate and save plots of projections with Matern kernel
...
for i in range(num_runs):
    ...
    projected_x = np.loadtxt(output_paths[i].joinpath('matern_dkl_projections.csv'), delimiter=',')
    ...
    plt.savefig(
        output_paths[i].joinpath('matern_dkl_projections.pdf'),
        dpi=dpi,
        bbox_inches='tight',
        pad_inches=0.1
    )
